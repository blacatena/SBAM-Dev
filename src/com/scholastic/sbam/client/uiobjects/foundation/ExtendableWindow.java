 ...
 */
/**
 * This class simply re-implements and extends the Window class, so that it can be properly extended.
 * 
 * Two problems were encountered.  
 * 
 * First, the class must extend Window instead of ContentPanel so that it can be registered with the WindowManager.
 * 
 * Second, for some reason the Window.closable field continues to matter (huh?) instead of the local closeable field when drawing the closable box on the window
 * (some other class must be casting this directly to a Window).
 * 
 */
@SuppressWarnings("deprecation")
public class ExtendableWindow extends Window {
  ...
  protected int initialWidth = 300;
  protected boolean modal;
  protected boolean blinkModal = false;
  protected boolean onEsc = true;
  protected boolean plain;
  protected boolean resizable = true;
  protected Layer ghost;
  protected ToolButton maxBtn, minBtn, moveBtn, resizeBtn;
  protected boolean maximized;
  protected ModalPanel modalPanel;
  protected Resizable resizer;
  protected Point restorePos;
  ...
  protected Boolean restoreShadow;
  protected Boolean restoreWindowScrolling;
  protected HandlerRegistration modalPreview;
  protected boolean dragging;

  /**
   * Creates a new window.
   */
  public ExtendableWindow() {
    baseStyle = "x-window";
    focusable = true;
    frame = true;
...
public void focus() {
  ...
  public void setZIndex(int zIndex) {
    ...
    if (modalPanel != null && modalPanel.isRendered()) {
        ...
        public void onPreviewNativeEvent(NativePreviewEvent event) {
          if (Element.is(event.getNativeEvent().getEventTarget())) {
            ...
            if (!resizing && !dragging && !tag.equalsIgnoreCase("html") && !tag.equalsIgnoreCase("body")
                && event.getTypeInt() != Event.ONLOAD && manager.getActive() == ExtendableWindow.this
                && (modalPanel == null || (modalPanel != null && !modalPanel.getElement().isOrHasChild(target)))
                && !ExtendableWindow.this.getElement().isOrHasChild(target) && fly(target).findParent(".x-ignore", -1) == null) {
...
protected void endDrag(DragEvent de, boolean canceled) {
    ...
    DeferredCommand.addCommand(new Command() {
      public void execute() {
        if (ExtendableWindow.this.eventPreview != null && ExtendableWindow.this.ghost != null) {
        	ExtendableWindow.this.eventPreview.getIgnoreList().remove(ExtendableWindow.this.ghost.dom);
        }
      }
    });
  }
  ...
  protected void onRender(Element parent, int pos) {
    ...
    eventPreview = new BaseEventPreview() {
      ...
      protected void onPreviewKeyPress(PreviewEvent pe) {
        WindowEvent we = new WindowEvent(ExtendableWindow.this, pe.getEvent());
        onKeyPress(we);
      }
